...
This note captures the repo-safe summary of the independent V4 audit feedback that drove V5. The original auditor `.docx` files remain local-only and are not committed to the repository.

## V5 Claim

V5 narrows the public claim to unauthorized-effects prevention under the `secure_v5` deployment profile. The claim is not "malicious text never reaches the model." The claim is that supported V5 surfaces do not directly mint effectful authority or advance privileged connector or trusted-state flows without a server-issued capability and a broker-issued approval envelope bound to the exact action.
...
Current local status on March 30, 2026:
