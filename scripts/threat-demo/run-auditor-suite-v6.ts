...
const suitePath = join(repoRoot, "config", "auditor", "v6_secure_claim_suite.json");
...
function normalizedCapabilityKinds(response: Record<string, any> | undefined): string[] {
  const candidates = response?.authorityCandidates ?? response?.capabilities;
...
function decisionOf(response: Record<string, any> | undefined): string {
  return String(
    response?.verdict?.decision ??
      response?.effectDecision?.decision ??
      response?.authorityDecision?.decision ??
...
function reasonCodesOf(response: Record<string, any> | undefined): string[] {
  const reasonCodes =
    ...
    response?.effectDecision?.reasonCodes ??
    response?.authorityDecision?.reasonCodes ??
...
function htmlCapture(testCase: SuiteCase) {
  return {
    surfaceType: "html",
    url: "https://safe.example/review",
    html: testCase.html,
    visibleText: testCase.visible_text,
    captureAttestation: defaultHtmlCapture()
  };
}

function authorityIdOf(candidate: Record<string, any>): string {
  return String(candidate.authorityId ?? candidate.capabilityId ?? candidate.ticketId ?? "");
}

function authorityDigestOf(candidate: Record<string, any>): string {
  return String(candidate.authorityDigest ?? candidate.capabilityDigest ?? candidate.ticketDigest ?? "");
}
...
async function signApprovalViaBroker(
  ...
  return (
    await brokerRuntime.issueApprovalSignature(broker.baseUrl, broker.authToken, {
      ...
      capabilityId: authorityIdOf(capability),
      capabilityDigest: authorityDigestOf(capability)
    })
  ).brokerSignature;
}
  ...
  try {
    ...
    for (const testCase of suite.cases) {
      ...
      const session = await postJson<{ session: Record<string, any> }>(daemon.baseUrl, "/v6/session/start", {
        ...
        taskPurposeClass: connectorCase ? "connector_setup" : "docs_navigation",
        allowedOrigins: ["https://safe.example", "https://docs.python.org"],
        allowedVerbs: ["navigate", "connector_prepare"],
        allowedPathClasses: connectorCase ? ["connector_setup"] : ["docs_navigation", "content_read"],
        forbiddenSinks: []
      });
      ...
      try {
        if (testCase.kind === "hidden_html") {
          ...
          const capability = observe.authorityCandidates?.[0] ?? observe.capabilities?.[0];
          observedCapabilities = normalizedCapabilityKinds(observe);
          ...
          const action = await postJson<any>(daemon.baseUrl, "/v6/action/evaluate", {
            sessionId: session.session.sessionId,
            authorityId: authorityIdOf(capability),
            authorityDigest: authorityDigestOf(capability),
            parameters: {}
          });
          observedDecision = decisionOf(action);
          observedReasonCodes = reasonCodesOf(action);
          assertCondition(action.effectDecision?.decision === "ALLOW", "Visible navigation authority did not allow the expected action.");
          ...
          assertCondition(
            observedReasonCodes.includes("AUTHORITY_DOES_NOT_REQUIRE_APPROVAL"),
            "Navigate approval rejection did not emit AUTHORITY_DOES_NOT_REQUIRE_APPROVAL."
          );
          ...
          assertCondition(legacy.error === "route_retired_use_v6", "Legacy route was not retired in secure_v6.");
